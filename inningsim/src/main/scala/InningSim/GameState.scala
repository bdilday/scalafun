...
case class GameState(base_outs: BaseOutState = BaseOutState(), score: Array[Int] = Array(0, 0)) extends EvolveState {
...
trait EvolveState {
  ...
  def evolve_state(ev: BatEvent): GameState = {
    ...
    val bo = ev match {
      case xout() => {
        BaseOutState(
          self.base_outs.bases,
          self.base_outs.outs + 1
        )
      }
      case xbb() => {
        val x1 = 1
    ...
    val delta_runs = BatEvents.runs_scored(self.base_outs, bo)
    val new_score = Array(self.score(0) + delta_runs, self.score(1))
